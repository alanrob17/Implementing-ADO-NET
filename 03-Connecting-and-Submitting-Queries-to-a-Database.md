...
Now, we have to declare a connection string, which is usually defined in the App.Config or Web.Config file, so its available in your application. The typical entry of a connection string is written below:
...
Now, if your connection string is pointing to a SQL Server database like "RecordDB", here is the connection string.

Establish connection string in Web.Config file using the code below:
...
```
    SqlConnection cn = new SqlConnection("Data Source=TIGER; Initial Catalog=RecordDB; User ID=User Name; pwd=User Password" Integrated Security="True");
```
...
If the connection string is stored in the config file with its name RecordDB, we can read as following:

```
    var cnstr = System.Configuration.ConfigurationManager.ConnectionStrings["RecordDB"].Connection String;
```
...
| **Cursor Location** | It gets or sets location of cursor service. |
...
| **Close** | Close method is used, when any current connection is open and then closed after completed execution. |
| **Open** | Open method is used, if current connection is closed before execution is started. You must have opened connection to run a query. |
| **Execute** | This method it is used to execute a query. A Statement, procedure or provider provides specific text for execution. |
...
| **CommitTransaction** | If current transaction execution is successfully completed, it ends the current transaction. |
...
Connections use valuable resources so you always need to close the connection as soon as you have finished executing your query. You follow this with ``cn.Dispose()`` which is a set of low level Microsoft utilities to dispose of any resourses being used.
...
We get our connection string from the following method.
...
We set a breakpoint on the _viewModel statement and when it runs we can see our connection string.

```
    Server=TIGER;Database=ADONETSamples;Trusted_Connection=Yes;Application Name=ADO.NET Samples
```

This is sent to our ``Connect(string cnstr)`` method where *cnstr* is our connection string.

We now run the ``GetConnectionInformation(SqlConnection cnn)`` method.
...
This example uses Integrated Security but we could also use SQLServer authentication mode.

```
private void OpenConnectionSQLServerAuth_Click(object sender, RoutedEventArgs e)
    {
      _viewModel.Connect("Server=TIGER;Database=ADONETSamples;User ID=sa;Password=mypassword;");
    }
```

## Employ a Using block

Instead of using ``cn.Close()`` and ``cn.Dispose()`` commands to remove assest sued to carry out our query we can use ``Using()`` blocks to dispose of our resources for us. An example of usage is shown below.
...
In the code above we have 4 arguments in our Sql statement so that means we will newd 4 parameters to receive those values. In our case those values are coming from a class that has those values.
...
It has our 4 input parameters and and 1 output parameter. Output parameters are commonly used for returning the next primary key generated by the database.

*Scope Identity* gives us the next generated primary key value.
